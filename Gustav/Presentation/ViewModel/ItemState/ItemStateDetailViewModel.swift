...
final class ItemStateDetailViewModel {
    // Category
    ...
    // MARK: - Init
    init(itemState: ItemState, itemStateUsecase: ItemStateUsecaseProtocol, itemQueryUsecase: ItemQueryUsecaseProtocol) {
        self.itemState = itemState
        self.selectedColor = itemState.color
    ...
    func action(_ input: Input) {
        ...
        case .didChangeTagColor(let tagColor):
            ...
            self.taskRemote?.cancel()
            self.taskRemote = Task {
                await self.changeItemStateColor(tagColor)
                onNavigation?(.reFetchItemStateList)
            }
        case .startChangeName:
        ...
        case .changedNameButton(let name):
            self.taskRemote?.cancel()
            self.taskRemote = Task {
                await self.updateItemStateName(name)
                onNavigation?(.reFetchItemStateList)
            }
        case .didTappedDeleteButton:
            self.taskRemote?.cancel()
            self.taskRemote = Task {
                await self.deleteItemState()
                onNavigation?(.delete)
            }
        }
    }
...
private extension ItemStateDetailViewModel {
    // 아이템 쿼리
    private func fetchItems() async {
        // 쿼리 조건 생성
        let itemQuery = ItemQuery(
            sortOption: .createdAt(order: .ascending),
            filters: [.itemState(itemState.id)],
            searchText: nil
        )
        ...
        let reult = await itemQueryUseCase
            .queryItems(
                workspaceId: self.itemState.workspaceId,
                query: itemQuery,
                pagination: nil
            )
    ...
    // 카테고리 이름 변경
    private func updateItemStateName(_ name: String) async {
        let updatedItemState = ItemState(
            id: self.itemState.id,
            workspaceId: self.itemState.workspaceId,
            indexKey: self.itemState.indexKey,
            name: name,
            color: self.itemState.color
        )
        
        let result = await itemStateUseCase.updateItemState(id: itemState.id, itemState: updatedItemState)
        
        switch result {
            case .success:
            self.itemState = updatedItemState
    ...
    // 카테고리 테그 컬러 변경
    private func changeItemStateColor(_ tagColor: TagColor) async {
        let updatedItemState = ItemState(
            id: self.itemState.id,
            workspaceId: self.itemState.workspaceId,
            indexKey: self.itemState.indexKey,
            name: self.itemState.name,
            color: tagColor
        )
        
        let result = await itemStateUseCase.updateItemState(id: self.itemState.id, itemState: updatedItemState)
    ...
    // 카테고리 삭제
    private func deleteItemState() async {
        let result = await itemStateUseCase.deleteItemState(id: self.itemState.id, workspaceId: self.itemState.workspaceId)
        switch result {
        case .success:
            emit(.delete)
        case .failure(let error):
