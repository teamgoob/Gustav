...
class ItemStateListViewController: UIViewController {
    private let contentView = ItemStateListView()  // 기본 뷰에 사용할 뷰
    private let loadingView = LoadingView()             // 로딩뷰
    private let viewModel: ItemStateListViewModel  // 뷰모델
    ...
    @objc private func endButtonTapped() {
        ...
        case .addItemState:
            break
        case .changeOrder:
            self.viewModel.action(.didTapreorderItemStateButton)
